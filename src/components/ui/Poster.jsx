...
export const Poster = ({ id, type, imagePath, title, releaseDate, rating }) => {
  ...
  const addToFavorites = () => {
    ...
    const movieObject = { id, type, imagePath, title, releaseDate, rating };
    setFavorites([...favorites, movieObject]);
    saveFavoritMovies([...favorites, movieObject]);
  ...
  const removeFromFavorites = () => {
    if (isFavorite) {
      ...
      saveFavoritMovies(favorites.filter((favorite) => favorite.id !== id));
    }
  };
  ...
  return (
    <div className='relative flex rounded w-posterWidth aspect-poster'>
      <Link to={`/${type}/${id}`} onClick={handleClick} className='w-posterWidth'>
        <div
          style={bgStyle}
      ...
      >
        ...
        <Link to={`/${type}/${id}`} onClick={handleClick}>
