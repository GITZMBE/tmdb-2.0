...
export const SeriesPoster = ({ series }) => {
  ...
  let { genre_ids: genreIds = [], name = "", first_air_date = "", vote_average = 0, poster_path: url = "" } = useMemo(() => {return seriesObject || {}}, [seriesObject]);
  ...
  const isFavorite = useMemo(() => favorites?.some((favorite) => favorite.id === series.id), [favorites, series.id]);
  ...
  const addToFavorites = () => {
    ...
    const seriesAndType = { ...series, type: "series" };
    setFavorites([...favorites, seriesAndType]);
    saveFavoritMovies([...favorites, seriesAndType]);
